...
interface Props {
    items: CompletedItem[];
    canUnlock?: boolean;
    onUnlock?: (item: CompletedItem) => void;
    lectureId?: string;
    examId?: string;
}

/** 정렬 키에서 "ip"를 제거 */
type SortKey = "name" | "id" | "startTime" | "duration" | "status" | "none";
type SortOrder = "asc" | "desc" | "none";

const CompletedListRenderer: React.FC<Props> = ({
    items,
    canUnlock = false,
    onUnlock,
    lectureId,
        ...
        newArr.sort((a, b) => {
            ...
            switch (sortKey) {
                ...
                case "startTime":
                    ...
                    break;
                case "duration":
                    result = (a.duration || "").localeCompare(b.duration || "");
                    break;
                case "status":
            ...
            <table className="min-w-full table-auto">
                <thead>
                    <tr>
                        ...
                        >
                            이름 {sortKey === "name" && sortOrder !== "none" && `(${sortOrder})`}
                        </th>
                        ...
                        >
                            학번 {sortKey === "id" && sortOrder !== "none" && `(${sortOrder})`}
                        </th>
                        ...
                        >
                            상태 {sortKey === "status" && sortOrder !== "none" && `(${sortOrder})`}
                        </th>

                        {/* IP 헤더는 정렬 불가 (no onClick, no cursor-pointer) */}
                        ...
                        >
                            시작 시간 {sortKey === "startTime" && sortOrder !== "none" && `(${sortOrder})`}
                        </th>
                        ...
                        >
                            종료 시간 {sortKey === "duration" && sortOrder !== "none" && `(${sortOrder})`}
                        </th>
                    </tr>
                </thead>
                ...
                <tbody className="divide-y">
                    {sortedItems.map((item, idx) => {
                        ...
                        const displayName = isLongName ? `${item.name.slice(0, 4)}...` : item.name;
                        ...
                        return (
                            ...
                            >
                                ...
                                <td className={`px-4 py-2 whitespace-nowrap ${isLongName ? "group relative" : ""}`}>
                                    {displayName}
                                    {isLongName && (
                                ...
                                {/* IP: 표시만, 정렬 없음 */}
                                <td
                                    className={`px-2 py-2 whitespace-nowrap text-gray-700 ${isLongIp ? "group relative" : ""
                                        }`}
                                >
                                    {displayIp}
                                    {isLongIp && (
                                    ...
                                    <div className="inline-flex items-center gap-3">
                                        ...
                                        {canUnlock && item.finishedByUser ? (
                                            <button
                                                ...
                                                className="ml-2 rounded bg-indigo-500 px-2.5 py-1 text-xs font-semibold text-white hover:bg-indigo-600"
                                            >
                                                해제
                                            </button>
                                        ) : null}
...
export default CompletedListRenderer;
